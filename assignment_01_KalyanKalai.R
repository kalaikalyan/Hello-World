...
## Use the values 6.1, 8.8, 7.7, 6.4, 6.2, 6.9, 6.6
 week1_sleep <-  c(Sun=6.1,Mon=8.8,Tue=7.7,Wed=6.4,Thu=6.2, Fri=6.9,Sat=6.6)
...
addstudent <- rbind(grades, student04)
addstudent
## Add a new assignment column with `cbind()`
assignment04 <- c(92.1, 84.3, 75.1, 97.8)
addassignment <- cbind(addstudent, assignment04)
addassignment
...
rownames(addassignment) <- students
colnames(addassignment) <- assignments
addassignment
## Total points for each assignment using `colSums()`
colSums(addassignment)

## Total points for each student using `rowSums()`
rowSums(addassignment)
